...
{
  // ros::NodeHandle nh_p("~"); 
  server_port_id_ = 27015;
  // server_ip_ = string("192.168.1.6"); 
  // server_ip_ = string("192.168.0.100"); 
  server_ip_ = string("192.168.100.2");
  // nh_p.param("sr_server_ip", server_ip_, string("127.0.0.1")); 
  // nh_p.param("sr_server_ip", server_ip_, string("192.168.1.2")); 
...
void* CSRTcpSocket::thread_SR_Recevicer()
{
  ...
  while(b_thread_on) // sym
  {
    TTimeStamp recv_start = getCurrentTime();
    res=TCPRecv(tcp_socket_,(char*)&rx,sizeof(rx),0);
    // ++number_of_frames;
    // printf("TCPRecv consume %d\n",res);
    if(res<=0)
    ...
    if(data)
    {
      ...
      TTimeStamp recv_end = getCurrentTime(); 
      printf("SR_tcp_socket.cpp: recv frame %d cost time %lf ms\n", n_cnt++, timeDifference(recv_start, recv_end)*1000.);
