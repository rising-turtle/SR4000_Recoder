...
string sr_data_version = "old"; // only record depth (z) and intensity (i), 
...
int writeData(int argc, char* argv[]); 

int main(int argc, char* argv[])
{
  ros::init(argc, argv, "sr4k_recorder"); 
  ros::NodeHandle n; 
...
int writeData(int argc, char* argv[])
{
  ...
  ros::NodeHandle np("~"); 
  np.param("save_path_dir", path_dir, path_dir); 
  np.param("sr4k_model", mode, mode); 
  np.param("sr_data_version", sr_data_version, sr_data_version); 
  np.param("srcam_ip", srcam_ip, srcam_ip); 
